 ...
 * needed to restore one ioshmultichoice qtype plugin
 *
 * @copyright  2010 onwards Eloy Lafuente (stronk7) {@link http://stronk7.com}
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class restore_qtype_ioshmultichoice_plugin extends restore_qtype_plugin {
    ...
    protected function define_question_plugin_structure() {
        ...
        // Add own qtype stuff.
        $elename = 'ioshmultichoice';
        // We used get_recommended_name() so this works.
        $elepath = $this->get_pathfor('/ioshmultichoice');
    ...
    /**
     * Process the qtype/ioshmultichoice element
     */
    public function process_ioshmultichoice($data) {
        global $DB;
        ...
        // qtype_ioshmultichoice_opts too.
        if ($questioncreated) {
            $data->questionid = $newquestionid;

            // It is possible for old backup files to contain unique key violations.
            // We need to check to avoid that.
            if (!$DB->record_exists('qtype_ioshmultichoice_opts', array('questionid' => $data->questionid))) {
                $newitemid = $DB->insert_record('qtype_ioshmultichoice_opts', $data);
                $this->set_mapping('qtype_ioshmultichoice_opts', $oldid, $newitemid);
            }
        }
    }
     ...
     * recoded pointing to all the restored stuff for ioshmultichoice questions
     *
    ...
    public static function define_decode_contents() {
        ...
        $contents[] = new restore_decode_content('qtype_ioshmultichoice_opts',
                $fields, 'qtype_ioshmultichoice_opts');
