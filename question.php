 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class qtype_ioshmultichoice_base extends question_graded_automatically
{
    ...
    public function apply_attempt_state(question_attempt_step $step)
    {
        ...
        foreach ($this->order as $ansid)
        {
            ...
            $a = new stdClass();
            $a->id = 0;
            $a->answer = html_writer::span(
                get_string('deletedchoice', 'qtype_ioshmultichoice'),
                'notifyproblem'
            );
    ...
    {
        if ($component == 'question' && in_array(
            $filearea,
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_ioshmultichoice_single_question extends qtype_ioshmultichoice_base
{
    public function get_renderer(moodle_page $page)
    {
        return $page->get_renderer('qtype_ioshmultichoice', 'single');
    }
    ...
    public function get_correct_response()
    {
        foreach ($this->order as $key => $answerid)
        {
            if (question_state::graded_state_for_fraction(
                $this->answers[$answerid]->fraction
            )->is_correct())
            {
    ...
    public function get_validation_error(array $response)
    {
        ...
        return get_string('pleaseselectananswer', 'qtype_ioshmultichoice');
    }
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_ioshmultichoice_multi_question extends qtype_ioshmultichoice_base
{
    public function get_renderer(moodle_page $page)
    {
        return $page->get_renderer('qtype_ioshmultichoice', 'multi');
    }
    ...
    public function summarise_response(array $response)
    {
        ...
        foreach ($this->order as $key => $ans)
        {
            ...
            if (array_key_exists($fieldname, $response) && $response[$fieldname])
            {
                $selectedchoices[] = $this->html_to_text(
    ...
    public function grade_response(array $response)
    {
        ...
        // If the user selects more than the specified amount, do not give them any marks.
        $responses = $this->get_num_selected_choices($response);
        $correctResponses = sizeof($this->get_correct_response());

        if ($responses > $correctResponses)
        {
            $fraction = 0;
        }
        else
    ...
    public function get_validation_error(array $response)
    {
        ...
        return get_string('pleaseselectatleastoneanswer', 'qtype_ioshmultichoice');
    }
    ...
    public function get_hint($hintnumber, question_attempt $qa)
    {
        ...
        if (
            $this->get_num_selected_choices($qa->get_last_qt_data()) >
